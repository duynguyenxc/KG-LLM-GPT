...
## 2026-09-23 - Capture missing-source abstracts without changing frozen runs

The previous goal turn only verified and explained existing human-review packets; classified as no progress. This continuation revisited the objective, proposal, relevant transcript and Richmond limitations, then searched nine known incomplete source records. Initial PubMed HTML retrieval succeeded for S002 only; other responses failed content checks. These attempts are preserved. One public NCBI EFetch request returned XML for all nine records, now retained byte-for-byte with nine separate abstract records under data/source_acquisition/20260923-pubmed/. All nine abstracts were read; eight DOI/PMID identities match, while S010 lacks a PubMed DOI and was checked by PMID/title/authors.

The readable report is outputs/research_audit/source-acquisition-20260923/index.html. It exposes baseline availability, captured text lengths, primary URLs and assistant review notes. Nine full abstracts are not nine full papers: zero new local full-text PDFs were acquired. S010 body text was readable through the web tool, but local PDF download returned 403 and graphical inspection failed. Restricted repository copies and catalogue records were not substituted for sources. No author request was sent.

Validation checked nine source-record hashes, six unchanged baseline hashes, all nine report cards, S010 navigation, no horizontal overflow and no severe browser errors; a screenshot was visually inspected. The raw XML is 50,534 bytes, SHA-256 4eee2c88aad5dd9ea7606927000e3551631afc6cd383db76e4385ec6754e3c74. The acquisition record preserves method, known retrieval date, timestamp limitation and builder identity. No production code changed, no model API call occurred and no new synthesis, comparison score or human judgment was generated.

Next: obtain inspectable full texts where lawful access permits, then freeze a distinct enriched corpus and measure source-coverage effects separately from algorithm changes. Do not resume a frozen run with substituted sources. Semantic pilot/evaluator-v2 execution remains conditional on credit restoration; independent reviewers are still unavailable. The full goal remains incomplete.


## 2026-09-23 - Freeze supplemented corpus and prepare 28 source packets

Previous turn acquired nine official abstracts (a6c2a42), classified as progress. Added corpus.py to turn that material into a verified, separate source snapshot. It validates raw XML, parsed abstract identity/content, input hashes and complete source-page populations before creating a new destination; refuses overwrite and preserves parent/acquisition snapshots and budget lineage. Only incomplete sources can be supplemented. No primary-source note or benchmark code enters the extraction packet.

Created outputs/runs/corpus-20260923-abstracts-v1: 19 unchanged full-text records, eight complete abstracts, and S009 retaining its partial PDF page plus an abstract. source_locators.json distinguishes source units from original PDF pages. The existing semantic runner accepted this version and prepared all 28 packets in outputs/runs/semantic-20260923-enriched-preparation-v1 without model calls. No new extraction, theory, score or human judgment exists. The original evidence loader cannot consume this version; full synthesis integration remains outstanding.

Validation: nine new synthetic cases and 75 total tests pass, with the pre-existing asyncio configuration warning; targeted Ruff F/I passes. Actual checks verified nine bundles, 28 packets, six unchanged baseline hashes and older pilot/evaluator code hashes. Browser checks covered the nine-row change table, S009 source display, overflow and severe errors; the report screenshot was inspected. The first ad-hoc QA attempt assumed the evaluator code hash was a dictionary and stopped; the corrected check handles its string plus dependency hashes and passed. QA is outputs/research_audit/corpus-enrichment-20260923-qa.json.

Next: expose source-unit locator types explicitly in downstream displays before publishing enriched citations, then execute a controlled pilot after credit restoration. The current frozen viewer still uses generic Source page labels. Separate source enrichment from method changes when measuring effects. Missing full texts, canonical/graph/synthesis integration, actual semantic/evaluator execution, proposal training experiments and independent human evaluation remain unresolved; the full goal is incomplete.


## 2026-09-23 - Carry explicit source locators into semantic extraction and review

Previous turn froze the supplemented corpus and prepared 28 packets (ab0ec3f), classified as progress. Added source_units.py to validate complete source-unit maps and distinguish PDF, partial-PDF, abstract and metadata locators. Updated the semantic runner to a new V2 prompt/code identity: source kind and original-page identity enter extraction, and audited quotation locators survive entity/assertion JSON, CSV, qualified graph and readable evidence links. Unknown units remain unresolved. Updated the independent correspondence exporter and UI to use the same validated source identity, without exposing AI critic judgments. No old output or review packet was regenerated.

Added replay_semantic.py to invoke verified saved entry-point code for existing runs, checking frozen code/config and active dependency hashes. Both V1 preparations replayed offline with all 24 and 74 files unchanged; the new V2 preparation also replayed without changes. The evaluator-v2 implementation/dependency hashes remain unchanged. Current semantic_pipeline.py must not be used to resume V1. Frozen budgets do not automatically include future paid runs, so shared accounting must be checked before later replay after new spending.

Prepared outputs/runs/semantic-20260923-enriched-preparation-v2 with all 28 source packets and zero API calls/candidates. Real S009 text visibly distinguishes its retained PDF page from PubMed abstract source unit 2. All model-produced semantic examples used for integration/UI checks are synthetic under tmp/source-unit-ui-qa/, not scientific outputs or human judgments.

Validation: eight additional parameterized locator/replay cases; 83 repository tests pass with the existing asyncio warning; targeted Ruff F/I passes. Tests reject missing/duplicate/inconsistent maps, invented PDF pages, bad PubMed IDs, changed locator snapshots and changed frozen dependencies; exercise CSV/graph propagation and correspondence-map agreement. Actual packet/input/code/baseline hashes passed. Browser checks exercised S009, a synthetic assertion citation and independent review; screenshots were inspected with no severe browser errors. QA: outputs/research_audit/source-unit-20260923-qa.json.

Next: execute a controlled semantic pilot and evaluator after restored credit, then inspect actual errors before full execution and graph/synthesis integration. Source enrichment and prompt changes must not be combined into an unsupported single-factor improvement claim. Missing full texts, canonical alignment, training experiments and actual independent human review remain outstanding. Full goal remains incomplete; no live paid process or newly verified scientific score.
