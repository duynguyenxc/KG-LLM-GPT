> **Current pointer (2026-09-23):** API access was restored and all 28 source extractions/audits completed (180 findings; 174 machine-eligible). Read [current run results](docs/research/CURRENT_RUN_RESULTS.md) and [research status](docs/research/RESEARCH_AUDIT_STATUS.md) for the live or terminal synthesis/comparison state. Reviewers are not available yet; the owner requested review preparation. Human validation remains pending. The historical claims and workspace paths below must not be reused as current facts.
